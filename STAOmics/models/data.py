...
from ..utils import config, get_rs, logged, processes, get_default_numpy_dtype
...
## # Ref: from torch_geometric.utils import k_hop_subgraph
def k_hop_subgraph(node_idx: Union[int, List[int], Tensor],
                   num_hops: int,
                   edge_index: Tensor,
...
class AnnDataset(Dataset):
    ...
    *adatas
    ...
    def data_configs(self, data_configs: List[DATA_CONFIG]) -> None:
        ...
        # self.edge_index = [torch.LongTensor(np.array(np.nonzero(data.uns['adj']))) for data in self.adatas]
        self.edge_index = [torch.LongTensor(data.uns['edgeList']) for data in self.adatas]
        self.adjs = [data.uns['adj'] for data in self.adatas]
        # self.adjs = [torch.FloatTensor(data.uns['adj'].todense()) for data in self.adatas]
        # self.inter_adjs = [torch.FloatTensor(data.uns['inter_adj'].todense()) for data in self.adatas]
    ...
    def __len__(self) -> int:
        if self.mode == 'train':
            return int(self.size / self.getitem_size)  # drop_last during training, avoid error
        else:
            return ceil(self.size / self.getitem_size) # default code
    ...
    def __getitem__(self, index: int) -> List[torch.Tensor]:
        ...
        # If use_obs_names=True, cells with the same name will be merged,
        # and the total number will be less than the sum of the two batches of data.
        ...
        new_idx_list = []
        items = []
        edge_index_list = [self.edge_index[ii] for ii in range(len(self.edge_index))]
        kk = 0
        for extracted_data in self.extracted_data: # (x, xrep, xbch, xlbl, xdwt)
            kk = kk + 1
            for ii, idx, data, edge_index in zip(np.arange(len(self.data_configs)), shuffle_idx, extracted_data, edge_index_list):# different omics

                if len(new_idx_list)==len(self.data_configs):
                    items.append(torch.as_tensor(self._index_array(data, new_idx_list[ii])))
                else:
                    # use edge_index to retrieve the neighbors of idx
                    # relabel=F means original global index are used
                    new_idx = k_hop_subgraph(torch.LongTensor(idx), num_hops=1, edge_index=edge_index,
                                             relabel_nodes=False, num_nodes=data.shape[0])

                    neighbor_idx = list(set(new_idx.numpy()) - set(idx))  # select index for neighbors
                    if len(neighbor_idx) > 0:
                        new_idx = np.concatenate([idx, neighbor_idx])  # append neighbor index to the minibatch spots
                    else:
                        new_idx = idx

                    items.append(torch.as_tensor(self._index_array(data, new_idx))) # retrieve subset data
                    new_idx_list.append(new_idx)

        ### reconstruct sparse adj from edge_index
        # edge_weight = torch.ones(edge_index.shape[1])
        # num_nodes = combined.shape[0]
        # adj_concat = sp.coo_matrix((edge_weight, (edge_index[0], edge_index[1])), shape=(num_nodes, num_nodes))


        ## minibatch spatial neighbor graph with neighbors
        for idx, adj in zip(new_idx_list, self.adjs): # retrieve submatrix
            # adj[np.ix_(idx, idx)] #
            # items.append(adj[idx][:, idx])    ### subset spatial neighbot adj
            items.append(torch.FloatTensor(adj[idx][:, idx].todense()))

        ###  minibatch raw spatial neighbor graph without neighbors, only self-loops
        for idx in shuffle_idx:
            items.append(torch.eye(idx.shape[0]))
